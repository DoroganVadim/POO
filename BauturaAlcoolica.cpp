...
BauturaAlcoolica::BauturaAlcoolica(const BauturaAlcoolica& b) : Lichid(b.denumire, b.densitate) {
	this->tarie = b.tarie;
}
void BauturaAlcoolica::setTarie(double tarie) {
	this->tarie = tarie;
}
std::istream& operator >>(std::istream& is, BauturaAlcoolica& l){
	l.in(is);
	return is;
}

std::ostream& operator <<(std::ostream& os, BauturaAlcoolica l) {
	l.out(os);
	return os;
}